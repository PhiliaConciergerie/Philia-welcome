...
//  - l'envoi automatique des photos (API /api/admin/uploader-photo)
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/admin/uploader-photo' && request.method === 'POST') {
      return handleUploaderPhoto(request, env);
    }
...
async function handleUploaderPhoto(request, env) {
  let body;
  try {
    body = await request.json();
  } catch {
  ...
  if (!slug || !filename || !contentBase64) {
    return jsonResponse({ error: 'Informations manquantes pour la photo.' }, 400);
  }
  if (!env.GITHUB_TOKEN) {
    return jsonResponse({ error: 'Publication non configurée (jeton GitHub manquant).' }, 500);
  }
  if (!/^[a-z0-9-]+\.(jpg|jpeg|png|webp)$/i.test(filename)) {
    return jsonResponse({ error: 'Nom de fichier invalide.' }, 400);
  }

  const filePath = `public/logements/${slug}/${filename}`;
  const headers = githubHeaders(env);
  ...
  try {
    ...
    const putBody = {
      message: `Ajout photo ${filename} (${slug})`,
    ...
    if (!putRes.ok) {
      const errText = await putRes.text();
      return jsonResponse({ error: `Échec de l'envoi de la photo (${putRes.status}) : ${errText}` }, 502);
    }

    return jsonResponse({ ok: true, path: `/logements/${slug}/${filename}` });
  } catch (e) {
    return jsonResponse({ error: "Erreur inattendue lors de l'envoi de la photo." }, 500);
  }
}
